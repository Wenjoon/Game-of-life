...
gamestate = 1
dead = []
#maybe this would be faster if everything (gamestate wise) was stored in one, 2D list with 0s, and 1s and updated with one loop
...
def checkneighbors(x,y):
    ...
    for i in range (-1,2):
        for j in range (-1,2):
            ...
            if x+i >= 0 and x+i < boardsize and y+j >= 0 and y+j < boardsize and [x+i,y+j] in alive:
                aliveneighbors += 1
    if [x,y] in dead and aliveneighbors == 3: #in keyword alert
        changestatus.append([x,y])
    if [x,y] in alive and (aliveneighbors < 2 or aliveneighbors > 3): #in keyword alert
        changestatus.append([x,y])
def genboard():
